...
class SessionManager:
    ...
    async def set_session_context(self, user_id: str, state: SessionState,
                                active_agent: Optional[str] = None,
                                ttl: int = 1800) -> None:  # 默认30分钟过期
        """设置用户会话上下文"""
        r = await self.get_redis()
        key = f'session:{user_id}:context'
    ...
    async def pause_session(self, user_id: str, active_agent: str,
                            pause_ttl: int = 300,
                            pause_context: str = '') -> None:
        """暂停多轮对话会话"""
        r = await self.get_redis()
        key = f'session:{user_id}:context'
    ...
    async def resume_session(self, user_id: str, agent_name: str, ttl: int = 1800) -> None:
        """恢复暂停的会话为多轮对话"""
        r = await self.get_redis()
        key = f'session:{user_id}:context'
