...
class BaseAgent(ABC):
    ...
    async def _run_tool_loop(self, system: str, messages: list[dict],
                             tools: list[dict], max_tokens: int = 2048) -> tuple[str, bool]:
        """通用工具调用循环。子类可复用。
        ...
        for _ in range(self._max_tool_rounds):
            ...
            for tc in resp.tool_calls:
                ...
                tool_results.append({
                    'role': 'tool', 'tool_call_id': tc.call_id, 'content': result_text,
                })
