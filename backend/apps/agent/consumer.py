        ...
        while self._running:
            try:
                messages = await bus.consume(
                    AGENT_TASKS, CG_AGENTS, _CONSUMER_NAME,
    ...
    async def _handle(self, msg_id: str, fields: dict) -> None:
        async with self._semaphore:
            ...
            try:
                ...
                logger.info('[AgentTaskConsumer] Handle task cancelled for task_id=%s', task_id)
                raise
            except Exception as e:
                logger.error('[AgentTaskConsumer] dispatch error task_id=%s: %s', task_id, e)
                # 先 ack 原消息（从 PEL 移除），再按策略重发或写 DLQ
                await bus.ack(AGENT_TASKS, CG_AGENTS, msg_id)
                await bus.nack_and_retry(
                    AGENT_TASKS, fields, retry_count
                )
