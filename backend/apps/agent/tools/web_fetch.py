...
class WebFetchTool(BaseTool):
    ...
    async def execute(self, url: str = None, extract_mode: str = 'markdown', max_chars: int | None = None, **kwargs) -> ToolResult:
        # 从 kwargs 中提取 url（处理 LLM 未显式传递的情况）
        if url is None:
            url = kwargs.get('url')
        if not url:
            return ToolResult(success=False, error='url 参数缺失，LLM 调用 web_fetch 时未提供 URL')
        ...
        if result is None:
            logger.warning('[WebFetchTool] 所有提取方式均失败 url=%s jina/readability/raw 全部出错，请检查网络/代理/JINA_API_KEY', url)
        return ToolResult(success=result is not None, data=result, error='' if result else f'所有提取方式均失败: {url}')
    ...
    async def _fetch_readability(self, url: str, extract_mode: str, max_chars: int) -> str | None:
        try:
