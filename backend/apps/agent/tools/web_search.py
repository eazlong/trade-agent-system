...
class WebSearchTool(BaseTool):
    ...
    async def execute(
        self, query: str = None, count: int | None = None, **kwargs
    ) -> ToolResult:
        ...
        if not query:
            return ToolResult(
                success=False,
                error="query 参数缺失，LLM 调用 web_search 时未提供搜索关键词",
            )
        provider = (
            (getattr(settings, "WEB_SEARCH_PROVIDER", "duckduckgo") or "duckduckgo")
            .strip()
            .lower()
        )
