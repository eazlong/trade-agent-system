...
class OKXDataSource(BaseDataSource):
    ...
    async def connect_websocket(self) -> bool:
        ...
        try:
            ...
            self._ws = await self._http_client.ws_connect(
                self.ws_endpoint, heartbeat=25, receive_timeout=30
            )
            ...
            logger.info(
                "[OKX] WebSocket connected: status=%s, subs=%d",
                status["status"],
    ...
    async def disconnect_websocket(self) -> bool:
        ...
        try:
            logger.info(
                "[OKX] disconnecting WebSocket, current_subs=%d",
    ...
    async def _send_subscribe(self, sub_info: Dict) -> None:
        ...
        msg = {
            "op": "subscribe",
            "args": [{"channel": channel, "instId": symbol, "instType": inst_type}],
        }

        await self._ws.send_str(json.dumps(msg))

    def _build_channel(
        self, data_type: DataType, interval: Optional[KlineInterval] = None
    ) -> str:
        """构建订阅频道"""
    ...
    async def unsubscribe(
        ...
        if (
            ...
            msg = {
                "op": "unsubscribe",
                "args": [
                    {"channel": channel, "instId": symbol_fmt, "instType": inst_type}
                ],
            }
            await self._ws.send_str(json.dumps(msg))
