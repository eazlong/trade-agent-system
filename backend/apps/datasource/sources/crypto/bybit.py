...
class BybitDataSource(BaseDataSource):
    ...
    async def _handle_websocket_message(self, msg: Dict, market_type: MarketType) -> None:
        ...
        for raw_data in data_list:
            if 'kline' in topic:
                ...
                self._monitor.record_data(self.name, symbol, 'kline', kline.get('timestamp', receive_time), receive_time)
                ...
                self._monitor.record_data(self.name, symbol, 'trade', trade.get('timestamp', receive_time), receive_time)
                ...
                self._monitor.record_data(self.name, symbol, 'ticker', ticker.get('timestamp', receive_time), receive_time)
    ...
    def _build_topic(self, symbol: str, data_type: DataType, interval: Optional[KlineInterval] = None) -> str:
        """构建订阅主题"""
