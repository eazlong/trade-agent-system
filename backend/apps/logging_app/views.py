...
def log_test_view(request):
    ...
    return Response({'status': 'ok', 'message': 'Test log written'}, status=status.HTTP_200_OK)
...
def _sse_event_reader(stream_key, last_id=None):
    ...
    if not r.exists(stream_key):
        r.xadd(stream_key, {'level': 'INFO', 'module': 'core', 'logger': 'system',
                            'message': 'SSE stream connected', 'trace_id': '', 'extra': '{}',
                            'ts': time.time()}, maxlen=10000, approximate=True)
                ...
                if messages:
                    for _, entries in messages:
                        for entry_id, fields in entries:
                            ...
                            try:
                                data['extra_data'] = json.loads(extra) if isinstance(extra, str) else extra
                            except Exception:
                                data['extra_data'] = {}
