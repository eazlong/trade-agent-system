...
class SystemLogHandlerTest(TestCase):
    ...
    def test_handler_emits_log(self, mock_redis):
        ...
        self.assertEqual(SystemLog.objects.filter(message__contains='Test log message').count(), 1)
...
class LogAPITest(TestCase):
    def setUp(self):
        ...
        SystemLog.objects.create(level='ERROR', module='trading', message='Order failed')
        SystemLog.objects.create(level='WARNING', module='riskguard', message='High leverage detected')
        SystemLog.objects.create(level='INFO', module='agent', message='Skill registered', trace_id='trace-001')
    ...
    def test_test_log_endpoint(self):
        ...
        try:
            resp = self.client.post(
                reverse('log-test'),
                data=json.dumps({'level': 'WARNING', 'message': 'Test warning', 'module': 'test'}),
