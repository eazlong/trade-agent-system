...
def _make_klines(count: int, base_price: float = 50000.0, volatility: float = 500.0) -> list[dict]:
    """生成模拟 K 线数据"""
    np.random.seed(42)
    klines = []
...
class TestRSI(unittest.TestCase):
    ...
    def test_downtrend_rsi_low(self):
        ...
        valid = result[~np.isnan(result)]
        self.assertTrue(len(valid) > 0)
        self.assertTrue(valid[-1] < 30, f"Downtrend RSI should be < 30, got {valid[-1]}")
...
class TestATR(unittest.TestCase):
    ...
    def test_basic(self):
        highs = np.array([105.0, 108.0, 103.0, 110.0, 107.0, 112.0, 109.0, 115.0, 108.0, 113.0,
                          106.0, 111.0, 108.0, 114.0, 109.0, 116.0, 110.0, 113.0, 107.0, 112.0])
        lows = np.array([95.0, 98.0, 93.0, 100.0, 97.0, 102.0, 99.0, 105.0, 98.0, 103.0,
                         96.0, 101.0, 98.0, 104.0, 99.0, 106.0, 100.0, 103.0, 97.0, 102.0])
        closes = np.array([100.0, 103.0, 98.0, 105.0, 102.0, 107.0, 104.0, 110.0, 103.0, 108.0,
                           101.0, 106.0, 103.0, 109.0, 104.0, 111.0, 105.0, 108.0, 102.0, 107.0])
...
class TestParallelComputation(unittest.TestCase):
    ...
    def test_parallel_performance_under_100ms(self):
        ...
        for r in results:
            self.assertIsNone(r['error'], f"Indicator {r['indicator_type']} failed: {r['error']}")
