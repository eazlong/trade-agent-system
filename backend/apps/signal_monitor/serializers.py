...
class SignalMonitorCreateSerializer(serializers.Serializer):
    ...
    trigger_type = serializers.ChoiceField(choices=['once', 'continuous'], default='once')
...
class SignalMonitorUpdateSerializer(serializers.Serializer):
    ...
    trigger_type = serializers.ChoiceField(choices=['once', 'continuous'], required=False)
    action_type = serializers.ChoiceField(
        choices=['notify', 'trade', 'notify_and_trade'],
        required=False,
    )
...
class SignalMonitorListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SignalMonitor
        fields = [
            'id', 'name', 'symbol', 'interval', 'source',
            'indicator_type', 'indicator_params', 'condition',
            'trigger_type', 'action_type', 'action_params',
            'status', 'last_triggered_at', 'trigger_count',
            'expires_at', 'backtest_result', 'created_at', 'updated_at',
        ]
        read_only_fields = [
            'id', 'status', 'last_triggered_at', 'trigger_count',
