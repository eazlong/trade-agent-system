...
class TestRiskGuardL2(unittest.TestCase):
    ...
    def test_pre_trade_check_circuit_open_rejects(self):
        ...
        with patch.object(self.guard, '_is_circuit_open', mock_cb.is_open):
            with patch.object(self.guard, '_get_daily_trade_count', AsyncMock(return_value=0)):
                with patch.object(self.guard, '_check_position_limit', AsyncMock(return_value=(True, ''))):
                    with patch.object(self.guard, '_check_drawdown', AsyncMock(return_value=(True, ''))):
                        result = asyncio.run(
    ...
    def test_pre_trade_check_daily_trade_limit_rejects(self):
        ...
        with patch.object(self.guard, '_is_circuit_open', AsyncMock(return_value=False)):
            with patch.object(self.guard, '_get_daily_trade_count', AsyncMock(return_value=50)):
                with patch.object(self.guard, '_check_position_limit', AsyncMock(return_value=(True, ''))):
                    with patch.object(self.guard, '_check_drawdown', AsyncMock(return_value=(True, ''))):
                        result = asyncio.run(
                            self.guard.pre_trade_check(request, 'user123')
    ...
    def test_pre_trade_check_position_limit_rejects(self):
        ...
        with patch.object(self.guard, '_is_circuit_open', AsyncMock(return_value=False)):
            with patch.object(self.guard, '_get_daily_trade_count', AsyncMock(return_value=0)):
                with patch.object(self.guard, '_check_position_limit', AsyncMock(return_value=(False, '单笔仓位 500.0% 超过上限 20%'))):
                    with patch.object(self.guard, '_check_drawdown', AsyncMock(return_value=(True, ''))):
                        result = asyncio.run(
                            self.guard.pre_trade_check(request, 'user123')
                        )
    ...
    def test_pre_trade_check_all_checks_pass(self):
        ...
        with patch.object(self.guard, '_is_circuit_open', AsyncMock(return_value=False)):
            with patch.object(self.guard, '_get_daily_trade_count', AsyncMock(return_value=5)):
                with patch.object(self.guard, '_check_position_limit', AsyncMock(return_value=(True, ''))):
                    with patch.object(self.guard, '_check_drawdown', AsyncMock(return_value=(True, ''))):
                        result = asyncio.run(
                            self.guard.pre_trade_check(request, 'user123')
...
class TestRiskGuardRecordOrder(unittest.TestCase):
    ...
    def test_record_success_calls_circuit_breaker(self):
        ...
        with patch('apps.riskguard.guard.CircuitBreaker', return_value=mock_cb):
            asyncio.run(
                RiskGuard().record_order_success('user123')
            )
    ...
    def test_record_failure_calls_circuit_breaker(self):
        ...
        with patch('apps.riskguard.guard.CircuitBreaker', return_value=mock_cb):
            asyncio.run(
                RiskGuard().record_order_failure('user123')
            )
