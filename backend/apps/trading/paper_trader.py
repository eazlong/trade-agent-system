...
class PaperTrader:
    ...
    async def place_order(self, request: OrderRequest) -> OrderResponse:
        ...
        if executed:
            logger.info(f'[PaperTrader] order executed: {request.symbol} {request.side} {request.quantity}')
            ...
            logger.info(f'[PaperTrader] order rejected: {request.symbol} {request.side} {request.quantity}')
            return OrderResponse(
                exchange_order_id=order_id,
                status='rejected',
    ...
    async def _execute_order(self, request: OrderRequest) -> bool:
        ...
        if request.side == 'buy':
            if self._balances.get('USDT', Decimal('0')) < amount:
                logger.warning(f'[PaperTrader] insufficient balance for buy order: {amount}')
                return False
            ...
            if symbol in self._positions:
                ...
                avg_price = (pos.avg_price * pos.quantity + price * request.quantity) / total_qty
            ...
            # 检查持仓
            if symbol not in self._positions or self._positions[symbol].quantity < request.quantity:
                logger.warning(f'[PaperTrader] insufficient position for sell order: {symbol}')
                return False
            ...
            if pos.quantity == 0:
                ...
                logger.info(f'[PaperTrader] position closed for {symbol}, P&L: {realized_pnl}')
            else:
                # 部分卖出，更新剩余持仓价值
                self._balances['USDT'] += (price * request.quantity)

        logger.info(f'[PaperTrader] balance: {self._balances}, positions: {list(self._positions.keys())}')
        return True
