"""
Standalone unit tests for the BigWaveStrategy class in backend/qtcore/strategy.py
"""
...
class BigWaveStrategyTestCase(unittest.TestCase):
    ...
    def test_analyse_data_long_signal_rise_percentage_too_low(self):
        ...
        data = self.long_signal_data.copy()
        # Set close price just below the threshold
        data['close'][-1] = data['open'][-1] * (1 + self.strategy.RISE_PERSENT * 0.9)
    ...
    def test_analyse_data_short_signal_rise_percentage_too_low(self):
        ...
        # Create data with low rise percentage
        data = self.short_signal_data.copy()
        # Set close price just above the threshold (small decrease)
        data['close'][-1] = data['open'][-1] * (1 - self.strategy.RISE_PERSENT * 0.9)
